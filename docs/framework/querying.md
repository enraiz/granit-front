...
function App() {
  return (
    <QueryProvider config={{ client: apiClient, basePath: '/api/patients' }}>
      <PatientList />
    </QueryProvider>
  );
}
...
| `config.basePath` | `string` | — | Préfixe des endpoints REST (ex : `'/api/patients'`) |
...
const {
  ...
  setSort,      toggleSort,
  setPresets,   setQuickFilters, toggleQuickFilter,
  setGroupBy,   setParams,       reset,
...
const {
  ...
  // Actions
  setInput,          selectField,      selectOperator,
  confirmValue,      addPresetToken,   addQuickFilterToken,
  addSearchToken,    removeToken,      clearAll,   cancel,
