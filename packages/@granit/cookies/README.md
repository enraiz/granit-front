<img src="../../../docs/images/granit-icon.svg" alt="" height="32" align="left" style="margin-right:10px" />

# @granit/cookies

Abstraction React pour le consentement cookies : contexte, hook `useCookieConsent` et interface fournisseur.

Fait partie du framework [granit-front](https://gitlab.digitaldynamics.be/digital-dynamics/granit-front).

## Installation

Consommé via `link:` protocol — voir la [documentation d'intégration](../../../README.md).

## Documentation

Voir la [documentation complète](../../../docs/framework/cookies.md).
